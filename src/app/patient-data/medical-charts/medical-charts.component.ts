...
@Component({
  ...
  templateUrl: './medical-charts.component.html',
  styleUrls: ['./medical-charts.component.css']
})
export class MedicalChartsComponent implements OnInit {
...
/*   patient: {name: string, taj: number, dateOfBirth: Date}; */
  ...
  private medChart: Chart; // a demoChart egy Chart típusú objektum
  ...
  ngOnInit() {
    ...
    if(this.templates[0]!=undefined)
    this.tempCopy=this.templates[0].content;
    if(this.templates1[0]!=undefined)
    this.tempCopy1=this.templates1[0].content;
  });
...
//    this.datePipe.transform(newContentName, 'yyyy-MM-dd');
  ...
  private createChart(disp, template, name) {
    if(this.medChart!=undefined)
    this.medChart.destroy();
    let labelName: string;
    let labels1: any[] = [];
    for(let i=3; i<disp.length;i++){
      labels1.push(disp[i]);
    }
    let data1: any[] = [];
    for(let i=0; i<template.length;i++){
      for(let j=0; j<template[i].content.length; j++){
        if(template[i].content[j].name==name){
          data1.push(template[i].content[j].value);
          if(labelName==null || template[i].content[j].unit!=null){
            labelName= template[i].content[j].unit;
            labelName = labelName + '   Ref. tart: ' + template[i].content[j].ref;
          }
        }
      }
    }

    this.medChart = new Chart(this.demoCanvas.nativeElement, {
      type: 'bar',
      data: {
        labels: labels1,
        datasets: [
  ...
  eventSelection(disp, template, event){
    this.createChart(disp, template, event);
   }
